# -*- coding: utf-8 -*-
"""Sales Agent | Cerebras and LiveKit. This code was adapted from a tutorial in a Colab notebook. Check out the origional below!
...
# Context directory will be created automatically by load_context()
...
The agent loads our context and uses it to answer user questions — accurately and without hallucinating.
...
> ⚠️ Note: All responses are spoken aloud, so we’ve added constraints to the prompt to avoid things like bullets or non-verbal symbols!
"""
...
async def multi_agent_entrypoint(ctx: JobContext):
    ...
    # Stream agent speech over data channel so frontend can read captions
    def _on_chat(msg):
        try:
        ...
        if hasattr(msg, "role") and msg.role == "assistant":
            try:
                session.room.local_participant.publish_data(
    ...
    session.on("conversation_item_added", _on_chat)
...
if __name__ == "__main__":
    ...
    async def _main():
        opts = WorkerOptions(entrypoint_fnc=multi_agent_entrypoint)
        # Explicitly set LiveKit Cloud URL
        opts.ws_url = LIVEKIT_URL
        worker = Worker(opts, devmode=True, register=True)
